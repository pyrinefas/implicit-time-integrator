...
tic;
f = @(t, u) [u(2)*u(3); u(1)*u(3); -2*u(1)*u(2)];  % Updated System of ODEs
dfdu = @(t, u) [0, u(3), u(2); u(3), 0, u(1); -2*u(2), -2*u(1), 0];  % Updated Jacobian matrix
u0 = [1; 2; -3];  % Updated Initial conditions
...
function u = gauss_legendre_newton_system(f, dfdu, u0, N)
    ...
    for i = 1:N
        ...
        for j = 1:1000  % We'll iterate a maximum of 10 times
            F = zeros(3, 3);
            J = eye(3);  % Jacobian matrix initialized to identity
